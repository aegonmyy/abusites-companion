  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const noteRes = await fetch("/api/notes", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          title: title.trim() || (mode === "text" ? rawText.slice(0, 60) : "Photo note"),
          sourceType,
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4" data-testid="new-note-form">
      <div>
        ...
        <p className="text-sm text-black/60 dark:text-white/60 mt-1">
          Paste text or snap a photo — the local model reads it and builds a summary, key
          concepts, and a short quiz.
        </p>
      </div>
      ...
      >
        {status === "generating" ? "Reading & summarizing…" : "Generate note"}
      </button>
    </form>
  );
}
